...
public class TaskModeCharge implements Task {

    private ReturningSetting returningSetting;

    private final RobotInfo robotInfo;

    public TaskModeCharge() {
    ...
    public void initTask(Intent intent) {
        ...
        CallingInfo.INSTANCE.getHeartBeatInfo().setCurrentTask(
                new TaskInfo(
                        currentTimeMillis,
                        currentTimeMillis,
                        TaskMode.MODE_CHARGE.ordinal(),
                        token,
                        PointCacheInfo.INSTANCE.getChargePoint().getSecond().getName()
                )
        );
    ...
    public Pair<String, String> getNextPointWithElevator() {
        ...
        return new Pair<>(chargePoint.getFirst(), chargePoint.getSecond().getName());
    }
